...
// Modifiers we can evaluate from a URL alone. `$document` is a content-type
// restriction that is always satisfied here, since the extension only ever
// cleans document URLs. A rule carrying anything else is skipped rather than
// applied with its restriction silently dropped.
const SUPPORTED = new Set(['removeparam', 'domain', 'denyallow', 'document']);

// Each bucket splits into `n`, the names of unconditional rules that just drop
// one parameter, and `r`, everything needing a closer look. The overwhelming
// majority land in `n`, where a lookup costs one set probe per parameter.
const rules = {
  version: 1,
...
// Rulesets are not committed. Fetch any that are missing so a fresh checkout
// builds with a single command; delete the directory to pull newer ones.
...
function parseLine(line, source) {
  ...
  // exception rule negates. '' is the naked form meaning every parameter.
...
// The regexp a rule tests each `name=value` pair against. `null` is the naked
// `$removeparam`, which takes every parameter. A plain name is normalized the
...
// Emit the index in its final runtime shape, so loading it costs nothing but
// evaluating the module. The `any` bucket holds hundreds of names and earns a
// Set; host buckets average two names each, where a plain array is both faster
// to search and cheaper to build.
function serialize() {
...
export const block = ${serializeIndex(rules.block)};

export const allow = ${serializeIndex(rules.allow)};
`;
}

// Rebuild the index with the values the runtime actually wants, then let
// javascript-stringify write them out as source. It emits regexp literals
// rather than `new RegExp(...)` calls, which keeps V8 free to compile them
// lazily, on first use.
function serializeIndex(index) {
  return stringify({
    any: runtimeBucket(index.any, true),
    hosts: Object.fromEntries(
      Object.entries(index.hosts).map(([host, b]) => [
        host,
...
// The `any` bucket holds hundreds of names and earns a Set; host buckets
// average two names each, where a plain array is both faster to search and
// cheaper to build.
